cat('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~',fill=T)
cat('Non-linear, State-Space Baseflow Separation Model',fill=T)
cat('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~',fill=T)
cat('This script will run the model for USGS sites using','parameters available from the file bf_params_usgs.csv and','streamflow data from USGS NWIS or stored in a local file as input.','The R package dataRetrieval must be installed to access NWIS data.', 'This script will load the package.', fill=T)
cat(fill=T)

cat('Follow the prompts, do not use quotation marks around your responses.',fill=T)
cat(fill=T)
